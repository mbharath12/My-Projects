...
logging.basicConfig(filename=os.path.join(PATH, 'testrail-'+str(time_stamp)+'.log'), format=LOG_FORMAT, level=logging.DEBUG)
...
def publish_results(api, testcases, run_id=0, plan_id=0, version='', publish_blocked=True):
    ...
    if run_id:
        if api.is_testrun_available(run_id):
            ...
            case_id_in_testrun_list = [str(tc['case_id']) for tc in testcases_in_testrun_list['tests']]
            testcases = [
