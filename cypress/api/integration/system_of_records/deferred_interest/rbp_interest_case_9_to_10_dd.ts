...
import DeferredInterestJSON from "../../../../resources/testdata/deferredinterest/rbp_interest_case_9_to_10.json";
...
//Test Cases Covered [Case 9 - Case 10]
//PP2756 - Borrower pays off according to the amortization schedule through end of term making all payments on time exact monthly prescribed payments - on time
//PP2757 - Borrower pays off according to the amortization schedule through end of term missing some payments and incurring late fees (exact monthly prescribed payments – some not on time)


TestFilters(["regression", "interestCalculation", "mixedinstallment" , "installment" ,"custom-fit" ,"rbp"], () => {
  let productID;
  let customerID;
  let accountID;
  let response;
  ...
  describe("Validate deferred interest calculation with standard installment product with different scenarios", function () {
    before(() => {
      ...
      DeferredInterestJSON.forEach((data) => {
        describe(`should have to create transactions and validate - '${data.tc_name}'`, () => {
          switch (data.processing_type) {
            ...
            case "getStatementID":
              it(`should be able to get the statement '${data.exp_line_item_type}'`, async () => {
                cy.wait(5000);                
