...
import transactionJSON from "../../../../resources/testdata/deferredinterest/deferred_interest_case_1_to_6.json";
...
//Test Cases Covered [Case 1 - Case 6]
//PP2747 - Borrower pays off balance before promo period ends and before the full loan term ends making all payments on time
//PP2748 - Borrower pays off balance before promo period ends and before the full loan term ends missing some payments and incurring late fees
//PP2749 - Borrower pays off balance after the promo period ends and before the full loan term ends making all payments on time
//PP2750 - Borrower pays off balance after the promo period ends and before the full loan term ends missing some payments and incurring late fees
//PP2751 - Borrower pays off balance after the promo period ends according to the amortization schedule making all payments on time (exact monthly prescribed payments - on time)
//PP2752 - Borrower pays off balance after the promo period ends according to the amortization schedule missing some payments and incurring late fees (exact monthly prescribed payments – some not on time)

TestFilters(["regression", "interestCalculation", "mixedinstallment"], () => {
  let productID;
  let customerID;
  let accountID;
  let response;
  ...
  describe("Validate deferred interest calculation with mixed installment product with different scenarios", function () {
    before(() => {
      ...
      //create new product
      const productJSONFile = "deferred_interest_product.json";
      ...
      transactionJSON.forEach((data) => {
        describe(`should have to create transactions and validate - '${data.tc_name}'`, () => {
          switch (data.transaction_type) {
            ...
            case "getStatementID":
              it(`should be able to get the components in statement '${data.exp_line_item_type}'`, async () => {
                cy.wait(5000);
            ...
            case "validateStatement":
              it(`should be able to validate the components in statement '${data.exp_line_item_type}'`, async () => {
            ...
            case "AMSchedule":
              it(`should have validate amort schedule`, async () => {
