...
public class TS14ValidateMultiLanguages {
	...
	public void test() throws IOException{
		...
		for(int index = 1;index <= mapAllDetails.size();index++){
			...
			if(mapquestionsDetails.get("Functionality") !=null && mapquestionsDetails.get("Functionality").equalsIgnoreCase("Multi Language")) {
				...
				if(mapquestionsDetails.get("Question Type").equalsIgnoreCase("Input Correct code")) {
					bstatus = ApplicationFunctions.doValidateMultilangugesbycorrectcode(mapquestionsDetails);
				}
				if(mapquestionsDetails.get("Question Type").equalsIgnoreCase("Empty")) {
					bstatus = ApplicationFunctions.doValidateMultilangugesbyemptycode(mapquestionsDetails);
				}
				if(mapquestionsDetails.get("Question Type").equalsIgnoreCase("Input Wrong code")) {
					bstatus = ApplicationFunctions.doValidateMultilangugesbyWrongcode(mapquestionsDetails);
				}
				...
				if(!bstatus && mapquestionsDetails.get("ExpectedResults").equalsIgnoreCase("Pass")){
					...
					ApplicationFunctions.navigatehomePage();
					continue;
				}
