...
public class TS11ValidateLockQuestionMemberwithGroup {
	...
	public void test() throws IOException{
		...
		for(int index = 1;index <= mapAllDetails.size();index++){
			...
			if(mapquestionsDetails.get("Functionality") !=null && mapquestionsDetails.get("Functionality").equalsIgnoreCase("Member In group")) {
				...
				loginDetails.put("User Name", mapquestionsDetails.get("User Name"));
				loginDetails.put("Password", mapquestionsDetails.get("Password"));

				//************ Validate Login Functionality *****************//

				bstatus = ApplicationFunctions.loginToApplication(loginDetails);
				if(!bstatus) {
				...
				bstatus = ApplicationFunctions.doVerifyLockSystemAsMemberOfGroup();
				...
				if(bstatus && mapquestionsDetails.get("ExpectedResults").equalsIgnoreCase("Pass")){
					test.log(Status.INFO, MarkupHelper.createLabel("Validate "+mapquestionsDetails.get("Functionality")+" Question Types", ExtentColor.GREEN)).pass(""+mapquestionsDetails.get("Functionality")+" "+mapquestionsDetails.get("Question Type")+" is validated  successfully");
				}
				if(bstatus && mapquestionsDetails.get("ExpectedResults").equalsIgnoreCase("Fail")){
