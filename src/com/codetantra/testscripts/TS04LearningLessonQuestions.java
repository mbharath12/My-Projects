...
public class TS04LearningLessonQuestions {
	...
	public void test() throws IOException{
		...
		for(int index = 1;index <= mapAllDetails.size();index++){
			...
			if(mapquestionsDetails.get("Functionality") !=null && mapquestionsDetails.get("Functionality").equalsIgnoreCase("Question Type")) {
				test = extent.createTest(""+mapquestionsDetails.get("TestCaseName")+"_Question Types", "<pre> "+"Validate Question Types"+"</pre>");
				bstatus = ApplicationFunctions.doVerifyQuestiontype(mapquestionsDetails);
				if(!bstatus && mapquestionsDetails.get("ExpectedResults").equalsIgnoreCase("Pass")){
					test.log(Status.INFO, MarkupHelper.createLabel("Validate Question Types", ExtentColor.RED)).fail("Unable to validate Question Types"+Messages.errorMsg, MediaEntityBuilder.createScreenCaptureFromPath(CommonFunctions.takeScreenShot(Global.driver)).build());
					continue;
				}
				if(!bstatus && mapquestionsDetails.get("ExpectedResults").equalsIgnoreCase("Fail")){
					test.log(Status.INFO, MarkupHelper.createLabel("Validate Question Types", ExtentColor.GREEN)).pass("Negative testcase-Cannot perform to Validate Question Types");
					continue;
				}
				if(bstatus && mapquestionsDetails.get("ExpectedResults").equalsIgnoreCase("Pass")){
					test.log(Status.INFO, MarkupHelper.createLabel("Validate Question Types", ExtentColor.GREEN)).pass("Question Types are validated  successfully");
					continue;
				}
				if(bstatus && mapquestionsDetails.get("ExpectedResults").equalsIgnoreCase("Fail")){
					test.log(Status.INFO, MarkupHelper.createLabel("Validate Question Types", ExtentColor.RED)).fail("Able to validated Question Types with negative Scenario", MediaEntityBuilder.createScreenCaptureFromPath(CommonFunctions.takeScreenShot(Global.driver)).build());
					continue;
				}
			}
		}
